...
Afin de lancer le site web, il faut tout d'abbord lancer le serveur, avec la commande ```python main.py```. Dans la console il y aura l'addresse du site ainsi que le message ***''Le coin de Namur' is on AIR!'*** afin d'indiquer que le serveur est bien lancé. Avant de lancer le serveur il faut installer tout les packages qui sont dans le fichier requirements.txt, cela peut être fait avec la commande ```pip install -r requirements.txt```.
